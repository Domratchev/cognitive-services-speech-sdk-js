...
export class FileAudioSource implements IAudioSource {
    ...
    public attach = async (audioNodeId: string): Promise<IAudioStreamNode> => {
        ...
        const streamReader = await this.upload(audioNodeId);
    ...
    private upload = async (audioNodeId: string): Promise<StreamReader<ArrayBuffer>> => {
        await this.turnOn();
        ...
        const chunk1 = this.privFile.slice(startOffset, endOffset);
